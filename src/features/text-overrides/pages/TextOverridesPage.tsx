...
import { formatShortcut, loadShortcutBindings, SHORTCUT_UPDATED_EVENT } from '@/shared/shortcuts/shortcutConfig';
import {
  readTextEditMode,
  readTextOverrides,
  removeTextOverride,
...
export function TextOverridesPage() {
  ...
  const [shortcutLabel, setShortcutLabel] = useState(() =>
    formatShortcut(loadShortcutBindings().toggle_text_edit_mode),
  );
      ...
      <header className="flex h-16 shrink-0 items-center justify-between border-b border-slate-100 bg-white px-6">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-xs text-slate-400">
            当前快捷键 {shortcutLabel}：点击页面文字后可修改显示文案，可在快捷键设置里更换。
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={exportTextOverrides}
            className="flex h-9 items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-600 hover:bg-slate-50"
          >
          ...
          <aside className="flex min-h-0 flex-col rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
            <div className="rounded-2xl bg-brand-light p-4">
              ...
              <div className="mt-2 text-xs font-medium leading-5 text-slate-500">
                启用 {enabledCount} 条，停用 {items.length - enabledCount} 条。
              </div>
            </div>

            <div className="mt-4 space-y-2">
              <button
                onClick={copySyncList}
                className="flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-brand px-4 text-sm font-bold text-white hover:bg-brand-dark"
              >
              ...
              <button
                ...
                className="flex h-10 w-full items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-600 hover:bg-slate-50"
              >
              ...
              <button
                onClick={clearAll}
                className="flex h-10 w-full items-center justify-center gap-2 rounded-xl border border-red-100 bg-white px-4 text-sm font-bold text-red-500 hover:bg-red-50"
              >
              ...
              <input
                ...
                onChange={(event) => {
                  ...
                  if (!file) return;
                  void importTextOverrides(file).catch((error) =>
                      ...
                      <div className="mb-3 flex items-center justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <div className="mt-1 truncate text-xs text-slate-400">
                            {item.path || '未记录页面'} · {item.updatedAt}
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <label className="block">
                          ...
                          <input
                            readOnly
                            value={item.original}
                            className="h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-500 outline-none"
                          />
                        </label>
                        <label className="block">
