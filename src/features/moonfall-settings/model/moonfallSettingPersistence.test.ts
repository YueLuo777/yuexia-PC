...
describe('moonfallSettingPersistence', () => {
  beforeEach(() => {
    localStorage.clear();
    vi.restoreAllMocks();
    delete window.xinyuexiaDatabase;
  });
  ...
  it('hydrates from database when local state is empty', async () => {
    const database = makeState('database-only', '数据库设定', '2026-05-25T01:00:00.000Z');
    window.xinyuexiaDatabase = {
      readCollection: vi.fn(async () => ({ ok: true, exists: true, data: [database] })),
      writeCollection: vi.fn(async (_collection, items) => ({ ok: true, exists: true, data: items })),
    } as unknown as Window['xinyuexiaDatabase'];

    const result = await hydrateMoonfallStateFromDatabase();

    expect(result.source).toBe('database');
    expect(result.state.settings[0].id).toBe('database-only');
    expect(JSON.parse(localStorage.getItem(MOONFALL_STATE_KEY) ?? '{}').settings[0].id).toBe('database-only');
  });

  it('persists to local cache and database collection together', async () => {
    const state = makeState('persisted', '持久化设定', '2026-05-25T01:00:00.000Z');
    const writeCollection = vi.fn(async (_collection, items) => ({ ok: true, exists: true, data: items }));
    window.xinyuexiaDatabase = {
    ...
    expect(writeCollection).toHaveBeenCalledWith('moonfallSettings', [expect.objectContaining({
      settings: [expect.objectContaining({ id: 'persisted' })],
    })]);
  });
});
