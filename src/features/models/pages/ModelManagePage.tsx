...
import {
  type ModelDraft,
  type ModelCardsPerRow,
  type ModelManagePageProps,
  MODEL_MANAGE_SETTINGS_KEY,
  MODEL_MANAGE_COLUMNS,
  MODEL_CARD_HEIGHT_CLASS,
  temperaturePresets,
  loadCardsPerRow,
  saveCardsPerRow,
  providerMeta,
  ...
  MODEL_POINTER_DRAG_ACTIVATION_DISTANCE,
  MODEL_POINTER_DRAG_ACTIVATION_DELAY_MS,
  MODEL_POINTER_DRAG_RETARGET_DISTANCE,
  MODEL_POINTER_DRAG_RETURN_DISTANCE,
  hasModelPointerRetargetedTooSoon,
  rememberModelPointerPreviewTarget,
  formatLogTime,
  ModelEditorModal,
  ModelManageSettingsModal,
} from '@/features/models/components/ModelManageParts';
      ...
      <div className={`flex min-h-0 flex-1 overflow-hidden px-5 py-4 ${cardsPerRow === 4 ? 'gap-4' : 'gap-5'}`}>
        <div className="min-w-0 flex-1 overflow-y-auto pr-1">
          ...
          >
            {previewModels.map((model, previewIndex) => {
              ...
              return (
                <div
                  ...
                  className={`model-card relative flex ${MODEL_CARD_HEIGHT_CLASS} flex-col overflow-hidden rounded-xl border bg-white transition-colors ${
            ...
            <button
              onClick={openAdd}
              className={`xy-radial-create-card model-card model-add-card flex ${MODEL_CARD_HEIGHT_CLASS} flex-col items-center justify-center rounded-xl border border-dashed border-[#08AACE]/50 bg-white text-[#08AACE] transition-colors hover:border-[#08AACE] hover:bg-[#E7F8FD]/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8FE4F2]`}
            >
        ...
        {!embedded && (
          <aside
            className={`model-failure-panel flex shrink-0 flex-col overflow-hidden rounded-xl border border-slate-200 bg-white ${cardsPerRow === 4 ? 'w-[280px]' : 'w-[300px]'}`}
          >
