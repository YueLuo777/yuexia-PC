import { AlertCircle, Eye, EyeOff, GripVertical, Plus, Server, X } from 'lucide-react';
...
export function ModelManagePage() {
  ...
  return (
    <div className="flex h-full flex-col bg-slate-50">
      ...
      <div className="flex min-h-0 flex-1 gap-5 overflow-hidden px-7 py-6">
        <div className="min-w-0 flex-1 overflow-y-auto pr-1">
          ...
          <div className="grid auto-rows-fr grid-cols-4 items-stretch gap-5">
            {models.map((model, index) => (
              <div
                ...
                className={`model-card relative flex h-full min-h-[296px] flex-col overflow-hidden rounded-[20px] border bg-white p-5 pr-[78px] transition-colors ${dragOverIndex === index ? 'border-brand ring-1 ring-brand' : 'border-slate-200'}`}
              >
                <button
                  onClick={() => updateModel(model.id, { locked: !model.locked })}
                  className={`absolute right-4 top-4 z-10 w-[72px] rounded-lg border px-2.5 py-1.5 text-center text-xs transition-colors ${
                ...
                <div
                  className="model-temp-slider absolute bottom-5 right-4 top-[96px] z-10 flex w-14 flex-col items-center rounded-full border border-slate-200 bg-slate-50 px-1.5 py-3"
                ...
                >
                  ...
                  <input
                    ...
                    onPointerCancel={() => setTemperatureDragId(null)}
                    onDragStart={(event) => event.preventDefault()}
                    className="h-full w-8 cursor-pointer accent-brand"
        ...
        <aside className="model-failure-panel flex w-[360px] shrink-0 flex-col overflow-hidden rounded-[20px] border border-slate-200 bg-white">
