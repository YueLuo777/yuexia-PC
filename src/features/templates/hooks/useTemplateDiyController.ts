...
export function useTemplateDiyController({
  initialStructure,
  onChange,
}: {
  ...
  const initialStructureRef = useRef<TemplateStructure>(cloneTemplateStructure(initialStructure));
  const [structure, setStructureState] = useState<TemplateStructure>(() => cloneTemplateStructure(initialStructureRef.current));
  const setStructure = (next: TemplateStructure) => {
  ...
  const resetStructure = () => {
    const next = cloneTemplateStructure(initialStructureRef.current);
    const nextDomain = next[0];
    const nextGroup = nextDomain?.groups[0];
    const nextEntry = nextGroup?.entries[0];
    setStructure(next);
...
export type TemplateDiyController = ReturnType<typeof useTemplateDiyController>;
