    ...
    >
      ...
      <footer
        className={`shrink-0 border-t border-slate-200 bg-white ${
          footerActions ? 'grid min-h-[72px] grid-cols-[minmax(0,1fr)_auto]' : 'flex h-[72px] items-center justify-between gap-4 px-4'
        }`}
        data-template-editor-footer="true"
      >
        <div className={`min-w-0 ${footerActions ? 'flex items-center justify-between gap-4 px-4 py-3' : 'contents'}`}>
        ...
        {footerActions ? (
          <div className="flex items-center gap-3 border-l border-slate-200 px-4 py-3" data-template-confirm-actions="true">
            {footerActions}
          </div>
        ) : null}
      </footer>
