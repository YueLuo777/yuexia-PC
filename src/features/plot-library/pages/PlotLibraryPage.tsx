        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={() => setShowRecycle(false)}>
          <div className="flex max-h-[82vh] w-[760px] flex-col overflow-hidden rounded-xl bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-4">
              {recycleItems.length === 0 ? (
                <div className="flex min-h-[260px] flex-col items-center justify-center text-gray-300">
                    ...
                    <article key={item.id} className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
                      <div className="flex items-start justify-between gap-4">
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="mt-2 line-clamp-2 text-xs leading-5 text-gray-500">{prepareModalText(item.content) || item.content}</p>
                        </div>
                        <div className="flex shrink-0 items-center gap-2">
                          <button
                            onClick={() => restoreItem(item.id)}
                            className="flex items-center gap-1 rounded-lg border border-brand/30 px-3 py-1.5 text-[11px] text-brand transition-colors hover:bg-brand-light"
                          >
                            <RotateCcw className="h-3.5 w-3.5" />
                            恢复
                          </button>
                          <button
                            onClick={() => permanentDeleteItem(item.id)}
                            className="flex items-center gap-1 rounded-lg border border-red-200 px-3 py-1.5 text-[11px] text-red-600 transition-colors hover:bg-red-50"
                          >
