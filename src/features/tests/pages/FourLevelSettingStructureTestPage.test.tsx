...
describe('FourLevelSettingStructureTestPage', () => {
  ...
  it('shows core settings as four entry and field pairs without a canvas', () => {
    ...
    expect(document.querySelectorAll('[data-four-level-display-row]')).toHaveLength(18);
    expect(screen.getByText('9 个三级设定 · 63 个四级设定')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: '作品定位' })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: '小说类型' })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: '世界层级与连接' })).toBeInTheDocument();
  ...
  it('switches second-level groups and keeps all fourth-level fields selectable', () => {
    ...
    expect(screen.getByText('4 个三级设定 · 28 个四级设定')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: '故事主线' })).toBeInTheDocument();

    const field = screen.getByRole('button', { name: '主角初始目标' });
    fireEvent.click(field);
    expect(field).toHaveAttribute('aria-pressed', 'true');
    expect(screen.getByText(/当前选中：剧情规划 ＞ 全书规划 ＞ 故事主线 ＞ 主角初始目标/)).toBeInTheDocument();

    fireEvent.click(screen.getByRole('button', { name: '人物设定' }));
    const secondLevelNavigation = screen.getByRole('navigation', { name: '设定二级分类' });
    expect(within(secondLevelNavigation).getByRole('button', { name: '主角人物' })).toHaveAttribute('aria-pressed', 'true');
    expect(screen.getByRole('button', { name: '主角档案' })).toBeInTheDocument();
  });
});
