...
export default function TokenUsagePage() {
  ...
  useEffect(() => {
    const pruneOrphanRecords = () => {
      pruneRecordsByModels(readModelSnapshot());
    };
  ...
  const stats = useMemo(() => {
    const data = Object.values(getStatsByModel(records));
  ...
  const totalInput = useMemo(
    () => records.reduce((sum, record) => sum + (record.inputTokens ?? 0), 0),
    [records],
  );
  const totalOutput = useMemo(
    () => records.reduce((sum, record) => sum + (record.outputTokens ?? 0), 0),
    [records],
  );
  const totalTokens = useMemo(
    () => records.reduce((sum, record) => sum + getRecordTotal(record), 0),
    [records],
  );
    ...
    <div className="flex h-full flex-col bg-gray-50">
      <header className="flex shrink-0 items-center justify-between border-b border-gray-100 bg-white px-5 py-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-bold text-gray-900">Token用量</h1>
            <p className="text-xs text-gray-400">合并展示模型调用记录、Token 消耗、成功失败与延迟数据。</p>
          </div>
          <span className="rounded-md bg-cyan-500 px-2 py-0.5 text-xs text-white">{totalCalls} 次调用</span>
        </div>
